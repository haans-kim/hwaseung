...
class CompanyWideModelingService:
    ...
    def setup_pycaret_environment(
        ...
        # 데이터 준비
        if use_augmentation:
            ml_data, data_info = self.augment_data(organization, target_size)
        else:
    ...
    def compare_models(self, organization: str, n_select: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            if data_size < 30:
                ...
                models_to_use = self.small_data_models[:2]  # 2개만
                n_select = min(2, n_select)
                safe_fold = 2
            elif data_size < 200:
                models_to_use = self.medium_data_models[:3]  # 3개만
                n_select = min(3, n_select)
                safe_fold = 3
            else:
                models_to_use = self.large_data_models[:5]  # 5개까지
                n_select = min(5, n_select)
                safe_fold = 3
